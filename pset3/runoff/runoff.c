...
void tabulate(void)
{
    ...
    for(int i = 0; i < voter_count; i++) {
        for(int j = 0; j < candidate_count; j++) {
            ...
            if(!candidates[idx].eliminated) candidates[idx].votes++;
            break;
        }
    }
...
bool print_winner(void)
{
    ...
    for(int i = 0; i < candidate_count; i++) {
        if(candidates[i].votes == biggest_vote) {
            ...
            if(winners == 1) name = candidates[i].name;
        }
    }
...
int find_min(void)
{
    ...
    for(int i = 1; i < candidate_count; i++) {
        if(candidates[i].votes < vote && !candidates[i].eliminated) vote = candidates[i].votes;
    }
...
{
    for(int i = 0; i < candidate_count; i++) {
        if(candidates[i].votes == min && !candidates[i].eliminated) candidates[i].eliminated = true;
    }
}
